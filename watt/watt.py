...
kernel_WATT_rearrange = '''
	extern "C" __global__ void kernel_WATT_rearrange(
		const int n,
		const float* input,
		float* output
	) {
...
kernel_WATT_updateOutput = '''
	extern "C" __global__ void kernel_WATT_updateOutput(
	  const int n,
	  const float* rbot0,
	  const float* rbot1,
	  float* top
...
kernel_WATT_updateGradFirst = '''
	#define ROUND_OFF 50000

	extern "C" __global__ void kernel_WATT_updateGradFirst(
	  const int n,
	  const int intSample,
	  const float* rbot0,
	  const float* rbot1,
...
kernel_WATT_updateGradSecond = '''
	#define ROUND_OFF 50000

	extern "C" __global__ void kernel_WATT_updateGradSecond(
	  const int n,
	  const int intSample,
	  const float* rbot0,
	  const float* rbot1,
...
class WATTFunction(torch.autograd.Function):
	@staticmethod
	def forward(self, first, second):
		...
		if first.is_cuda == True:
			...
			cupy_launch('kernel_WATT_rearrange', cupy_kernel('kernel_WATT_rearrange', {
				'input': first,
				'output': rbot0
			}))(
			...
			cupy_launch('kernel_WATT_updateOutput', cupy_kernel('kernel_WATT_updateOutput', {
				'rbot0': rbot0,
				'rbot1': rbot1,
				'top': output
			}))(
		...
		if first.is_cuda == True:
			if gradFirst is not None:
				for intSample in range(first.shape[0]):
					...
					cupy_launch('kernel_WATT_updateGradFirst', cupy_kernel('kernel_WATT_updateGradFirst', {
						'rbot0': rbot0,
						'rbot1': rbot1,
			...
			if gradSecond is not None:
				for intSample in range(first.shape[0]):
					n = first.shape[1] * first.shape[2] * first.shape[3]
					cupy_launch('kernel_WATT_updateGradSecond', cupy_kernel('kernel_WATT_updateGradSecond', {
						'rbot0': rbot0,
						'rbot1': rbot1,
						'gradOutput': gradOutput,
						'gradFirst': None,
